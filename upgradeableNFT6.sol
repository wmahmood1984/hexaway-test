...
contract Helper is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address initialOwner) public initializer {
        ...
        packages.push(
            Package(2, 20 ether, 30 * daysSeconds, 5, 700 ether, 0, 10, 3)
        ); //15 //10
        packages.push(
            Package(3, 25 ether, 60 * daysSeconds, 10, 1200 ether, 0, 15, 4)
        ); //40
        packages.push(
            Package(4, 50 ether, 90 * daysSeconds, 15, 2200 ether, 0, 20, 5)
        ); //100
        packages.push(
            Package(5, 165 ether, 120 * daysSeconds, 20, 5500 ether, 0, 25, 6)
        ); //300
    ...
    function register(address _ref, address _user) public {
        ...
        require(users[_user].referrer == address(0), "2");
        require(_referrer != _user, "3");
    ...
    function buyPackage(uint8 id, address _user) public {
        uint amount = packages[id].price;
        require(checkEligibility(_user, id), "4");
        // require(
    ...
    function buyNFT(
        ...
        (uint forDis) = goDistribute(_nft, _nft._owner, _user);
    ...
    function mint(string memory _uri, address _user, uint _nextTokenId) public {
        require(
        ...
        if (NFTMayBeCreated) {
            ...
            goDistribute(tx1, nftused[0]._owner, _user);
            tx1.premium = 0;
            tx1.price = 50 ether;
            removeFirst2();
    ...
    UUPSUpgradeable
{
    ...
    function register(address _referrer) public {
        ...
        helper.register(_referrer, msg.sender);
    }

    function buyPackage(uint8 id) public {
        uint allowance = paymentToken.allowance(msg.sender, address(this));
        paymentToken.transferFrom(msg.sender, address(helper), allowance);
        helper.buyPackage(id, msg.sender);
    }
    ...
    function mint(string memory _uri) public {
        ...
        helper.mint(_uri, msg.sender, _nextTokenId);
